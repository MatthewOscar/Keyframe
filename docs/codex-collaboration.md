...
This document records actual product decisions and the division of work behind
Keyframe. It is not a generated session transcript.
...
- Make developer-recorded demonstrations the primary workflow: show GPT-5.6
  what to build and how, then have it apply the evidence in a target repository
  and run the tests.
...
- After core acceptance, add local project and plugin compatibility for Claude
  ...
  server or the Codex/GPT-5.6 reasoning flow.
...
- audit the product requirements and current MCP/plugin surface;
...
- create documentation, eval prompts, CI, and release automation; and
...
Keyframe itself does not call an OpenAI model. The reference developer workflow
...
This is the headline eval in `evals/cases.json`. GPT-5.6 is responsible for the
reasoning and code change, not for decorative summarization.

## Ongoing human responsibilities

- Select and audit any public demonstration video's redistribution rights.
...
- Keep public examples free of credentials, private paths, and unlicensed
  media.
